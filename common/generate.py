...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        help="Increase EPICS_CA_SERVER_PORT by 2 for each deployed IOC.",
        default=False,
        action="store_true",
    )
    parser.add_argument(
        "--base-epics-ca-port",
        help="Initial EPICS CA server port. It will increase by 2 for every ioc.",
        type=int,
        default=5064,
    )
    parser.add_argument(
        "--epics-caputlog-addr",
        default="0.0.0.0",
        help="EPICS Caputlog remote address.",
    )
    parser.add_argument(
        "--epics-caputlog-port", default="7011", help="EPICS Caputlog remote port."
    )
    parser.add_argument(
        "--epics-log-addr", default="0.0.0.0", help="EPICS Log remote address."
    )
    parser.add_argument(
        "--epics-log-port", default="7011", help="EPICS Log remote port."
    )
    ...
    defaults = {
        ...
        "LOG_ADDR": args.epics_log_addr,
        "LOG_PORT": args.epics_log_port,
        "CAPUTLOG_ADDR": args.epics_caputlog_addr,
        "CAPUTLOG_PORT": args.epics_caputlog_port,
    }
