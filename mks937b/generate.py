...
def generate(args, defaults):
    ...
    for sector in sectors:
        ...
        res += template_top.safe_substitute(defaults,
                                            CD=cd, STREAM_PROTOCOL_PATH=stream_protocol_path, IP_ADDR=ip_addr,
                                            IP_ASYN_PORT=ip_asyn_port, EPICS_CA_SERVER_PORT=epics_ca_server_port, )
        ...
        for device in devices:
            if device:
                ...
                with open(os.path.join(dir_name, 'server/protocol/' + proto_name + '.proto'), 'w+') as file:
                    file.write(
