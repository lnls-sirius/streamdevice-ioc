...
from mks937b.consts import DATA_MKS
...
def get_beagle(f_name, ip_addr, devices, ip_asyn_port="IPPort0", scan="1"):
    return {  # A Sector
        "f_name": f_name,
...
for _ip, values in DATA_MKS.items():
    if len(values) > 32:
        logger.error(
            "More than 32 devices are set for the {} network. Values {}.".format(
                _ip, values
            )
        )
        continue
    ...
    for val in values:
        devs.append(
            get_device(
                val["Dispositivo"],
                val["RS485 ID"].zfill(3),
                val["Configuracao"].split(" "),
    ...
    sectors.append(get_beagle(_ip + ":5002", _ip + ":5002", devs))
