...
# UDP/IP ioc for communication between the EPICS IOC and a DCM SE-10 environment temperature and
# humidity monitoring system.

# This Python program should be executed with two parameters. The first is the port of the UDP
# ioc. The other is the IP address of the DCM SE-10 module.
...
udp_ioc_address = ("0.0.0.0", UDP_PORT)
udp_ioc_socket = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
udp_ioc_socket.bind(udp_ioc_address)
...
while True:
    ...
    data, address = udp_ioc_socket.recvfrom(512)
    ...
    if data:
        ...
        answer += "\n"
        udp_ioc_socket.sendto(answer, address)
